...
import numpy as np
from peaks.core.fileIO.loaders.SES import _load_SES_data, _load_SES_metalines, _load_SES_manipulator_coords, _SES_find
...
def _load_Bloch_data(fname):
    ...
    if data['scan_type'] is None:
        ...
        # If dim0 is a dimension in data, we have at least one unidentified mapping coordinate
            ...
            Bloch_to_peaks_conventions = {'X': 'x1', 'Y': 'x2', 'Z': 'x3', 'P': 'polar', 'T': 'tilt', 'A': 'azi'}
            ...
            if 'dim1' in list(data):
                ...
                all_manipulator_coords = _load_SES_manipulator_coords(metadata_lines, unique=False)
                ...
                if abs(all_manipulator_coords[mapping_dims[0]][1] - all_manipulator_coords[mapping_dims[0]][0]) < 0.001:
...
def _load_Bloch_metadata(fname, scan_type):
    ...
    metadata['beamline'] = 'MAX IV Bloch'
