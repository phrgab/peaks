...
def _load_I05_metadata(fname, scan_type, loc):
    ...
    if file_extension == '.nxs':
        ...
        if loc == 'Diamond I05-nano':
            ...
            metadata['defocus'] = _h5py_find_coord(f, defocus_handles, 'defocus', num_dp=2)
            ...
            metadata['ana_polar'] = _h5py_find_coord(f, ana_polar_handles, 'ana_polar', num_dp=2)
        ...
        metadata['temp_cryo'] = _h5py_find_coord(f, temp_cryo_handles, 'temp_cryo', num_dp=2)

        # Extract the command to run the scan
        scan_command_handles = ['entry1/scan_command/']
        metadata['scan_command'] = _h5py_find_attr(f, scan_command_handles, 'scan_command', str)
