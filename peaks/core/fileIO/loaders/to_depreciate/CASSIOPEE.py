...
from peaks.core.fileIO.loaders.to_depreciate.SES import (
    _load_SES_data,
    _load_SES_metalines,
    _SES_find,
)
    ...
    if file_extension == "":
        ...
        file_list_ROI = natsort.natsorted(
            [
                ...
                if "ROI1" in item and isfile(join(fname, item))
            ]
        )
        file_list_i = natsort.natsorted(
            [
                item
                for item in os.listdir(fname)
                if "_i" in item and isfile(join(fname, item))
            ]
        )
        ...
        if abs(hv_values[-1] - hv_values[0]) > 1:  # Should be an hv scan
            ...
            scan_type = "FS map"
            spectrum = np.zeros(
                (len(polar_values), len(theta_par_values), len(KE_values))
            )
...
def _load_CASSIOPEE_metadata(fname, scan_type):
    ...
    if scan_type == "hv scan" or scan_type == "FS map":
        ...
        pol_index = int(
            _SES_find(
                additional_metalines, "Polarisation [0:LV, 1:LH, 2:AV, 3:AH, 4:CR]"
            )
        )
        ...
        try:
            metadata["tilt"] = round(
                float(_SES_find(additional_metalines, "tilt (deg)")), 3
            )
            metadata["azi"] = round(
                float(_SES_find(additional_metalines, "phi (deg)")), 3
            )
...
@_register_location
class _CassiopeeConventions(_BaseARPESConventions):
    loc_name = "SOLEIL CASSIOPEE"
    loader = _load_CASSIOPEE_data
    metadata_loader = _load_CASSIOPEE_metadata
    ana_type = "I"
    polar_name = "theta"
    tilt_name = "tilt"
    azi_name = "phi"
