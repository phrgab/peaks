#Functions to load data from the ALBA LOREA HR beamline into an xarray, or for the logbook maker
...
def load_lorea_data(file, logbook, **kwargs):
    ...
    #determine scan type
    if 'photon_energies' in analyser_keys: #should be a photon energy scan
        scan_type = "hv scan"
        ...
        if 'sapolar' in analyser_keys: #should be a FS manipulator polar map
            scan_type = "FS map"
        elif 'defl_angles' in analyser_keys:  #should be a FS deflector map
            scan_type = "defl map"
        else:
    ...
    #get xarray metadata
    if logbook == False:
        meta_list = get_lorea_metadata(f,scan_type,logbook)
    
    #get logbook metadata
    else:
        sample_upload, scan_timestamp = get_lorea_metadata(f,scan_type,logbook)
    ...
    if 'photon_energies' in analyser_keys: #should be a photon energy scan
        hv = np.round(f['entry1/data/photon_energies'][()],2) #Extract the photon energy, with rounding
        ...
        if 'sapolar' in analyser_keys: #should be a FS manipulator polar map
            ...
            deflector_x = f['entry1/data/defl_angles'][()] #extract the deflector mapping dimension
            #create a DataArray with the measured map 
            ...
            if 'saz' in analyser_keys and 'sax' in analyser_keys: #should be a spatial map
                ...
                temperature = f['entry1/sample/temperature'][()] #extract sample temperature
...
def get_lorea_metadata(f,scan_type,logbook):
    ...
    #assign metadata
    meta_list = {}
    sc_name = h5py_str(f['entry1/title'])
    try:
        meta_list['scan_name'] = sc_name.split('/')[-1]
    except:
        meta_list['scan_name'] = sc_name
    meta_list['scan_type'] = scan_type
    meta_list['sample_description'] = None
    meta_list['eV_type'] = 'kinetic'
    meta_list['beamline'] = 'ALBA LOREA'
    ...
    # polarisation
    try:
        f_pol = f['entry1/instrument/insertion_device/beam/final_polarisation']
    ...
    # number of sweeps
    try:
        meta_list['sweeps'] = int(f['entry1/instrument/analyser/number_of_scans'][0])
    except:
        try:
    ...
    # Dwell
    try:
        meta_list['dwell'] = float(np.round(f['entry1/instrument/analyser/time_per_scan'][0],2))
    except:
    ...
    try:
        if scan_type == 'defl map':
            deflector_x = f['entry1/instrument/analyser/defl_angles'][()] #extract the deflector mapping dimension
            ...
            try:
                meta_list['defl_perp'] = float(f['entry1/instrument/analyser/defl_angles'][0])
            except:
