# based on Display Panel of Edgar Abarca Morales
# minor by Tommaso Antonelli in order to interface with fit_GUI8
# so far just a proof of principle: still a lot to develop here
...
from peaks.core.GUI.display_panel.feedDPfromLoaders import feedDPfromLoaders


################################################################################

# Class to create a SelectDC_GUI object
class SelectDC_GUI(object):

    #--------------------------------------------------------------------------#

    # This class creates a Display Panel to select the DC used in the fitting
    ...
    def __init__(self, SelectDC_GUI):

        # Store the framework of the Display Panel
        self.SelectDC_GUI = SelectDC_GUI
    ...
    def GUI_setup(self):
        ...
        self.SelectDC_GUI.setObjectName("SelectDC_GUI")
        self.SelectDC_GUI.resize(871, 801)

        # Define central widget within main window
        self.centralwidget = QtWidgets.QWidget(self.SelectDC_GUI)
        ...
        # Define MDC/EDC dropdown menu
        self.DCsel = QtWidgets.QComboBox(self.centralwidget)
        self.DCsel.setGeometry(QtCore.QRect(745, 440, 151, 26))
        self.DCsel.setEditable(False)
        self.DCsel.setCurrentText("MDC")
        self.DCsel.addItems(["MDC","EDC"])
        ...
        self.SelectDC_GUI.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(self.SelectDC_GUI)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 860, 24))
        self.menubar.setObjectName("menubar")
        self.SelectDC_GUI.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(self.SelectDC_GUI)
        self.statusbar.setObjectName("statusbar")
        self.SelectDC_GUI.setStatusBar(self.statusbar)

        # Initialize widget's labels
        self.GUI_retranslate()
        QtCore.QMetaObject.connectSlotsByName(self.SelectDC_GUI)
    ...
    def GUI_retranslate(self):
        ...
        self.SelectDC_GUI.setWindowTitle(_translate("SelectDC_GUI","Display panel"))
        self.Followme.setText(_translate("SelectDC_GUI","Follow me"))
        self.Bilateral.setText(_translate("SelectDC_GUI","Bilateral Link"))
        self.LabelHSpan.setText(_translate("SelectDC_GUI","Span"))
        self.LabelStep.setText(_translate("SelectDC_GUI","Step"))
        self.LabelCsrMode.setText(_translate("SelectDC_GUI","Cursor mode: Normal"))
        self.DimorPix.setText(_translate("SelectDC_GUI","Dim/Pix"))
    ...
    def GUI_resize(self):
        ...
        ResizeItem(self.SelectDC_GUI,self.MainPlot)
        ResizeItem(self.SelectDC_GUI,self.MDCPlot)
        ResizeItem(self.SelectDC_GUI,self.EDCPlot)
        ResizeItem(self.SelectDC_GUI,self.CursorStats)
        ResizeItem(self.SelectDC_GUI,self.Followme)
        ResizeItem(self.SelectDC_GUI,self.Bilateral)
        ResizeItem(self.SelectDC_GUI,self.HCutOff)
        ResizeItem(self.SelectDC_GUI,self.LCutOff)
        ResizeItem(self.SelectDC_GUI,self.Invert)
        ResizeItem(self.SelectDC_GUI,self.Cmaps)
        ResizeItem(self.SelectDC_GUI,self.DCsel)
        ResizeItem(self.SelectDC_GUI,self.SpanX)
        ResizeItem(self.SelectDC_GUI,self.DeltaX)
        ResizeItem(self.SelectDC_GUI,self.SpanY)
        ResizeItem(self.SelectDC_GUI,self.DeltaY)
        ResizeItem(self.SelectDC_GUI,self.LabelX)
        ResizeItem(self.SelectDC_GUI,self.LabelY)
        ResizeItem(self.SelectDC_GUI,self.LabelHSpan)
        ResizeItem(self.SelectDC_GUI,self.LabelStep)
        ResizeItem(self.SelectDC_GUI,self.allX)
        ResizeItem(self.SelectDC_GUI,self.allY)
        ResizeItem(self.SelectDC_GUI,self.LabelCsrMode)
        ResizeItem(self.SelectDC_GUI,self.DimorPix)
        ResizeItem(self.SelectDC_GUI,self.Files)
    ...
    def updateMDC(self,csr_num):
        ...
        if self.ispany == 0:
            mdc_temp = self.z[:,self.find_nearest(self.y,self.cursors[csr_num-1].data['pos'][0][1])]
            self.MDCs[csr_num-1]=self.MDCPlot.plot(self.x, mdc_temp, pen=self.sidecs[csr_num-1])
        ...
        # If the y-integration is non-zero perform the integration
            ...
            # If the indexes are not equal average the MDCs within the integration-crosshair
                ...
                mdc_temp = np.sum(self.z[:,a:b+1],axis=1)/(b-a+1)
                self.MDCs[csr_num-1]=self.MDCPlot.plot(self.x,mdc_temp, pen=self.sidecs[csr_num-1])

        #define the MDC to send to the fit panel (MDC2FP)
        self.MDC2FP = xr.DataArray(mdc_temp,
                                    coords=[self.files[0][self.files[0].dims[0]]],
                                    dims=[self.files[0].dims[0]])

        self.MDC2FP.attrs = self.files[0].attrs
        # Retrieve selected DC to pass to fit panel (FP)
        if self.DCsel.currentText() == "MDC":
            self.send2FP = self.MDC2FP
    ...
    def updateEDC(self,csr_num):
        ...
        if self.ispanx == 0:
            edc_temp = self.z[self.find_nearest(self.x,self.cursors[csr_num-1].data['pos'][0][0]),:]
            self.EDCs[csr_num-1]=self.EDCPlot.plot(edc_temp,self.y, pen=self.sidecs[csr_num-1])
        ...
        # If the x-integration is non-zero perform the integration
            ...
            # If the indexes are not equal average the EDCs within the integration-crosshair
                ...
                edc_temp = np.sum(self.z[a:b+1,:],axis=0)/(b-a+1)
                self.EDCs[csr_num-1]=self.EDCPlot.plot(edc_temp,self.y, pen=self.sidecs[csr_num-1])

        #define the EDC to send to the fit panel (EDC2FP)
        self.EDC2FP = xr.DataArray(edc_temp,
                                    coords=[self.files[0][self.files[0].dims[1]]],
                                    dims=[self.files[0].dims[1]])

        self.EDC2FP.attrs = self.files[0].attrs
        # Retrieve selected DC to pass to fit panel (FP)
        if self.DCsel.currentText() == "EDC":
            self.send2FP = self.EDC2FP
